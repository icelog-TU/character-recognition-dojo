...
if (includeChars) {
  for (const char of lesson.newChars ?? []) {
    ...
    addJob(
      ...
      zhuyin
        ? `This is single-character audio. The target character is ${char}, pronounced with Taiwan zhuyin ${zhuyin}. Say ${char} exactly once, clearly and naturally, with a brief pause after it.`
        : `This is single-character audio. Say ${char} exactly once, clearly and naturally, with a brief pause after it.`,
    );
  }
}
