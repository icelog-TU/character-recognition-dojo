...
- Include the mandatory `Image style and cast requirements` block from `docs/LESSON_VISUAL_CAST_SOP.md`. This block must name the L058 style references, the current cast identity anchors, recurring `我` / `你` / `他` identity rules, side-by-side comparison requirement, the rule that semantic correctness is not enough, the requirement to reject style/cast drift before commit, the per-image final-report format (`S01 style-lock PASS, cast PASS ...`), and the stop condition for inaccessible references or repeated style/cast drift.
