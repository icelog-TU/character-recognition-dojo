...
- The `curriculum:package-intake` result. A final report that says complete while the gate fails, or while repo files still say `partial-package`, `needs-rework`, `Do not integrate`, `NOT COMPLETED`, `FAIL`, or `unresolved`, is invalid and must be corrected before Release receives it.
...
- Pre-merge package preview URL only if useful, using `ref=<branch-or-full-commit-sha>`. Label it clearly as "pre-merge package preview, not the final main review queue". Prefer the full pushed commit SHA. If using a branch ref with `/`, URL-encode it (`codex%2Fl###-complete-package`); never provide an unencoded `ref=codex/l###-complete-package` URL.
...
- If browser automation/manual playback QA could not run because Codex browser control crashed, timed out, could not identify the browser URL, or could not operate local media playback, report that as a tooling failure and follow `docs/CURRICULUM_PRODUCTION_SOP.md` browser automation fallback rules. If the teacher manually reviewed the pushed pre-merge package, include the exact URL, immutable commit SHA, reviewed scope, automation failure reason, teacher PASS or repair findings, and the technical checks that passed. Do not ask the teacher to perform pre-merge review as a normal requirement; teacher subjective image/audio review belongs to the post-merge repair queue unless the teacher explicitly asks for pre-merge approval.
