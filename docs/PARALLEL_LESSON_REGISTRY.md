...
|---|---|---|---|---|---|---|---|---|---|
| L156 | 風 / normal lesson | ready-blocked-by-dependency | Production thread B | `codex/l156-complete-package` / `3b2a3ca6b4ac85205d0fd53500510713801fd173` | L154:等; L155:再 | 等 再 | `curriculum-workflow/lesson-requests/L156.json`; `curriculum-workflow/generated/L156-generation-packet.md`; `curriculum-workflow/drafts/L156-draft.json` | `curriculum-workflow/audio-inbox/L156/`; `public/assets/lessons/L156/` | Complete package prepared with request, packet, draft, images, OpenAI audio, Stage 4 G02/G05 audio, and AI alignment. `npm run verify` passes. Production JSON/planner/ledger remain blocked until L154 and L155 enter `origin/main`. Audio review pending teacher OK: https://icelog-tu.github.io/character-recognition-dojo/tools/audio-review.html?unit=L156&ref=3b2a3ca6b4ac85205d0fd53500510713801fd173. Check with `npm run audio:review-status -- --unit L156 --ref 3b2a3ca6b4ac85205d0fd53500510713801fd173`. |
...
3. **Uploaded / pushed branch:** after pushing a task branch or any remote work for that lesson.
   ...
   - If teacher audio approval is required, `Notes` must include the permanent audio-review URL and say either `audio-review pending teacher OK` or `audio-review OK` with the reviewed commit SHA.
