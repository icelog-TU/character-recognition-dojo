...
- App name: `認字練功房`
- Current reviewed lessons: L001-L063
- L063 introduces `開`.
...
- L006-L063 include Stage 4 fixed sentence games after picture-supported listening.
...
- Production account sync uses Firebase Auth accounts, learning profiles under `accounts/{uid}/profiles/{profileId}`, and generated device IDs under `accounts/{uid}/devices/{deviceId}`.
- A customer/family account may have at most three active devices.
- Device names are editable labels for humans only. Do not use the label as an ID, authorization key, or merge key.
- Progress belongs to profiles, not devices. A device stores `activeProfileId`; two devices using the same profile sync the same progress.
- Parent devices may switch between child profiles. Child devices may be left on one profile.
...
- Settings must not show "general mode", "free browsing", or a manual legacy device-code field. Normal users should only see account sign-in, profile selection, editable profile/device labels, and system-generated device ID.
...
- For `teach-character`, production game data must set `targetCharIndex` and dedicated `teachAudio.prefixSrc` / `teachAudio.suffixSrc` files. Generate prefix audio from exactly the Han text before `targetCharIndex`, and suffix audio from exactly the Han text after `targetCharIndex`; omit the side that has no text. Do not create these files by cutting, muting, splicing, replacing, or overdubbing the full sentence audio.
- For `teach-character`, the helper's pre-target playback and the stitched replay prefix must stop before the target character audio begins. If the prefix includes the target audio, the result will sound like the target was spoken twice. The suffix must start after the target character and must not skip the first suffix character.
- Treat the old full-sentence `charTimings` split as a legacy fallback only. New or touched Stage 4 second-round `teach-character` work must use dedicated `teachAudio` assets and phone playback QA.
...
L001-L063 are reviewed and built.
L063 introduces `開`.
