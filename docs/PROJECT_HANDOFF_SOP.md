...
- Current reviewed lessons: L001-L417
- L417 introduces `如`.
...
Two-character word lessons are allowed only when the teacher explicitly chooses a natural learning unit that should not be split, such as `朋友`. They still consume exactly one `L###` lesson order and use `newChars` with two single Han characters. Both characters become learned at that same lesson order. Stage 1 may present the word as the child-facing target, but Stage 2 and Stage 4 interactions remain single-Han-character interactions; do not create two-character chunk cards or whole-word blank cards.

Stage 2 target-card rule: a one-character lesson uses a six-card grid with 3 target cards for the current lesson character and 3 distractor/review cards; the stage completes after the child taps the 3 target cards. A two-character word lesson still uses a six-card grid, but uses 2 target cards for each current lesson character plus 2 distractor/review cards; the stage completes after the child taps all 4 target cards. For example, `朋友` should show two `朋` cards, two `友` cards, and two old-character cards; tapping both `朋` cards and both `友` cards completes Stage 2. The first two-character lesson after this SOP change is a pilot; pre-merge card-count simulations should be recorded as pilot verification notes, not used as the sole reason to stop package production.
...
- Production cannot call a branch complete while package files still say `partial-package` or while required images/audio/timings/Stage 4 assets are missing. The pushed branch must pass package intake before it is handed to Release as complete.
