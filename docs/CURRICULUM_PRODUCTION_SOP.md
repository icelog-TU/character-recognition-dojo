...
- `G03 missing-character` is a three-choice single-Han game. It must provide exactly one correct option and exactly two distractor options, for three visible option buttons total. Every option `text` must be exactly one Han character, option texts must be distinct, and the correct option must match the single missing Han character. Do not ship a one-button `missing-character` round.
...
- Stage 4 spoken guidance must avoid Mandarin words that browser TTS commonly misreads in this context. For `teach-character`, do not say `請教我念`, `請教牠念`, or `卡在不會念的字`; use unambiguous wording such as `請你幫我念`, `請你幫牠念`, or `停在不會念的字`.
...
- In `teach-character`, keep cues distinct and minimally repetitive: the intro and ready cue say `請按住紅框的字不放`; the post-press cue says `聽到鈴聲，就大聲念出來`. Do not say `按一下` for the red-framed target, because the interaction requires holding until the recording cue. The helper's pre-press flow should stop after the child knows which action to take.
...
- On the final Stage 4 round, after the child presses `領取獎勵` and the reward is claimed, the UI must visibly show the red `下一課` button and white `回首頁休息` button without requiring the teacher or child to hunt below the fold. Mobile/tablet QA must include this final reward-navigation state, with the floating playback bar visible if applicable.
