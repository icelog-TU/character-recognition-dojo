...
- 注音：皮=ㄆㄧˊ
- 目標句數：5
- 教師備註：Teacher-approved final sentences from Production Handoff L286. Production note: G02 targetCharIndex corrected from handoff value 4 to actual Han index 3 for 皮 in S01.
...
- 如果句子有數量，圖片必須清楚符合該數量。
- 優先使用明亮背景與清楚主體。
- 圖片目標路徑格式：`/assets/lessons/L286/images/L286-S01.webp`
...
- 本課每個新字都要建立一個單字音訊。
- 單字音訊目標路徑格式：`/assets/lessons/L286/audio/char-字.m4a`
- 聲音：自然、清楚、適合幼兒的臺灣華語。
- 讀 `spokenText`，不要讀顯示標點。
- 不要把句音訊做成逐字拼接。
- 句音訊目標路徑格式：`/assets/lessons/L286/audio/L286-S01.m4a`
...
```json
{
  "src": "/assets/lessons/L286/audio/L286-S01.m4a",
  "durationMs": 1200,
  "charTimings": [
...
```json
{
  ...
  "newChars": ["皮"],
  "zhuyin": {"皮":"ㄆㄧˊ"},
  "charAudio": {"皮":"/assets/lessons/L286/audio/char-皮.m4a"},
  "title": "皮",
  "requiredRounds": 5,
  "sentences": [
    {
