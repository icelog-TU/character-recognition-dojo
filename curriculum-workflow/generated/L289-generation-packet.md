...
- 注音：忍=ㄖㄣˇ
- 目標句數：5
- 教師備註：Production C complete-package handoff for L289 忍. Corrected handoff: latest origin/main is formally through L277, so L283 破, L284 口, L285 傷, L286 皮, L287 痛, and L288 受 are provisional dependencies. Do not merge production JSON until L283-L288 exist in latest origin/main. Use the approved sentences exactly. Use Taiwan Traditional Chinese and Taiwan zhuyin only. spokenText must equal text with punctuation removed. No Han characters outside allowedChars may appear in text, spokenText, displayLines, focusChar, or Stage 4 option text. Images may show small injuries, discomfort, foot pain, and uncomfortable heat, but must be child-appropriate: no gore, no severe injury, no medical horror, and no dangerous heatstroke. Generate standalone AI character audio for 忍 as char-u5fcd.m4a; do not cut it from sentence audio. Generate sentence audio, G02 teachAudio prefix/suffix, and G05 wrong-choice audio as independent OpenAI TTS whole utterances.
...
- 如果句子有數量，圖片必須清楚符合該數量。
- 優先使用明亮背景與清楚主體。
- 圖片目標路徑格式：`/assets/lessons/L289/images/L289-S01.webp`
...
- 本課每個新字都要建立一個單字音訊。
- 單字音訊目標路徑格式：`/assets/lessons/L289/audio/char-字.m4a`
- 聲音：自然、清楚、適合幼兒的臺灣華語。
- 讀 `spokenText`，不要讀顯示標點。
- 不要把句音訊做成逐字拼接。
- 句音訊目標路徑格式：`/assets/lessons/L289/audio/L289-S01.m4a`
...
```json
{
  "src": "/assets/lessons/L289/audio/L289-S01.m4a",
  "durationMs": 1200,
  "charTimings": [
...
```json
{
  ...
  "newChars": ["忍"],
  "zhuyin": {"忍":"ㄖㄣˇ"},
  "charAudio": {"忍":"/assets/lessons/L289/audio/char-忍.m4a"},
  "title": "忍",
  "requiredRounds": 5,
  "sentences": [
    {
