...
const results = [];
for (const name of ['char-u821e', 'L450-G02-prefix', 'L450-G02-suffix', 'L450-S02']) {
  const file = `public/assets/lessons/L450/audio/${name}.m4a`;
  const bytes = fs.readFileSync(file);
  const trials = [];
  const prompt = name === 'char-u821e' ? '這是臺灣華語單一漢字音。課程候選字是「舞」，請只轉寫實際聽到的漢字。' : null;
...
fs.writeFileSync('curriculum-workflow/generated/L450-short-audio-audit.json', `${JSON.stringify(results, null, 2)}\n`);
