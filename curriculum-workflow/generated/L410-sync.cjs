...
for(const file of fs.readdirSync('public/assets/lessons/'+unit+'/audio').filter(f=>f.endsWith('.m4a'))){const src='/assets/lessons/'+unit+'/audio/'+file,p='public'+src;
 ...
 const last=a.charTimings.at(-1);if(!file.startsWith('char-')&&durationMs-last.endMs>300&&speechEndMs>last.endMs){report.timingAdjustments.push({file,reason:'Final speech decay extends beyond ASR word endpoint; retain decoded final syllable envelope',before:{...last},afterEndMs:speechEndMs});last.endMs=speechEndMs;}
