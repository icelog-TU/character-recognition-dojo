...
const d=read('curriculum-workflow/drafts/L381-draft.json'),r=read('curriculum-workflow/lesson-requests/L381.json');
const han=s=>[...s].filter(c=>/\p{Script=Han}/u.test(c));
const base=JSON.parse(cp.execFileSync('git',['show','250ca4a3361a15104ba19158bb1b0710dac43ecc:src/curriculum/sample-lessons.json'],{maxBuffer:32e6}));
const allowed=[...new Set(base.lessons.flatMap(l=>l.newChars).concat(['已','近','接','送','連','傳']))];
assert.equal(allowed.length,385);assert.deepEqual(r.generationConstraints.allowedChars,allowed);
...
for(const s of d.sentences){
 ...
 assert.equal(han(s.text).length,10);assert.equal(han(s.text).join(''),s.spokenText);assert(han(s.text).includes(s.focusChar));assert(han(s.text).every(c=>allowed.includes(c)));
 audio.set(s.audio.src,{...s.audio,text:s.spokenText});
}
for(const w of ['傳','連','送','接','近','已','經'])coverage[w]=d.sentences.reduce((n,s)=>n+s.text.split(w).length-1,0);
assert.deepEqual(coverage,{'傳':4,'連':2,'送':2,'接':2,'近':1,'已':1,'經':1});
...
assert.deepEqual(d.sentenceGames.map(g=>g.sentenceId),['L381-S01','L381-S02','L381-S04','L381-S03','L381-S05']);
assert.deepEqual(d.sentenceGames.map(g=>g.targetCharIndex),[2,5,2,3,3]);
assert.deepEqual(d.sentenceGames[3].missingIndexes,[6,7,8,9]);
assert.deepEqual(d.dependsOnLessons,['L376','L377','L378','L379','L380']);
assert.deepEqual(d.provisionalLearnedChars,['已','近','接','送','連']);
...
const g3=d.sentenceGames[2];assert.deepEqual(g3.options.map(o=>o.text),['傳','說','聽']);assert.equal(g3.options.filter(o=>o.correct).length,1);assert.deepEqual(g3.missingIndexes,[2]);
for(const c of d.newChars)audio.set(d.charAudio[c],{character:true});assert.equal(audio.size,10);
const out={unit:'L381',coverage,allowed:allowed.length,audio:[],images:[]};
for(const [src,m]of audio){
 ...
 if(m.character){assert(dur>=700&&dur<=3500&&max>=-35);}else{
  ...
  for(const [i,t]of m.charTimings.entries()){assert.equal(i,t.charIndex);assert(t.endMs>t.startMs&&t.endMs<=dur&&t.startMs>=0);assert(t.endMs-t.startMs>=80);if(i)assert(t.startMs>=m.charTimings[i-1].endMs);}
 }
...
if(process.argv.includes('--final')){
 ...
 const packet=fs.readFileSync('curriculum-workflow/generated/L381-generation-packet.md','utf8');
...
fs.writeFileSync('curriculum-workflow/generated/L381-technical-qa.json',JSON.stringify(out,null,2)+'\n');console.log(JSON.stringify(out,null,2));
