...
const action=process.argv[2],only=process.argv[3],dp='curriculum-workflow/drafts/L404-draft.json';
const shared=path.resolve('src/curriculum/sample-lessons.json');
const d=JSON.parse(read(dp,'utf8'));
const originalFetch=globalThis.fetch;
globalThis.fetch=async(...args)=>{
 if(String(args[0]).includes('/audio/speech')){const b=JSON.parse(args[1].body);b.instructions+=' 本課「弱」讀ㄖㄨㄛˋ，第四聲與尾音完整。「強」讀ㄑㄧㄤˊ。「勇者」讀ㄩㄥˇ ㄓㄜˇ。片段「點」讀ㄉㄧㄢˇ，不要加入其他字。使用自然臺灣華語，只念輸入文字。';args[1]={...args[1],body:JSON.stringify(b)};}
 if(String(args[0]).includes('/audio/speech')){const b=JSON.parse(args[1].body);if(b.input==='點'){b.instructions='Read exactly one Traditional Chinese character in clear natural Taiwan Mandarin: 點, ㄉㄧㄢˇ, third tone. Say the complete single syllable once at normal speaking volume, including initial consonant and full vowel with third tone. No greeting, no explanation, no other words. Leave a brief quiet tail after the syllable.';args[1]={...args[1],body:JSON.stringify(b)};}}
 if(String(args[0]).includes('/audio/transcriptions')&&args[1].body.get('file')?.name==='L404-G02-suffix.m4a')args[1].body.append('prompt','點');
 ...
 fs.mkdirSync('tmp/L404/transcripts',{recursive:true});write('tmp/L404/transcripts/'+name+'.json',JSON.stringify(raw,null,2)+'\n');
 const eq={'写':'寫','让':'讓','从':'從','个':'個','这':'這','画':'畫','后':'後','学':'學','们':'們','传':'傳','来':'來','连':'連','经':'經','图':'圖','给':'給','电':'電','帮':'幫','话':'話','饭':'飯','间':'間','里':'裡','声':'聲','这':'這','个':'個','妈':'媽','对':'對','说':'說','听':'聽'};
 Object.assign(eq,{'见':'見','书':'書','师':'師','脚':'腳','选':'選','双':'雙','单':'單','红':'紅','纸':'紙','过':'過','开':'開','强':'強','点':'點'});
...
if(action==='generate'&&only){
 ...
 else if(only==='char-u5f31'){d.sentences=[];d.sentenceGames=[];}
...
fs.writeFileSync=function(file,data,...args){
 if(path.resolve(String(file))===shared){
  ...
  if(action==='align'){
   ...
   for(const g of result.sentenceGames)if(g.type==='choose-pronunciation')for(const o of g.options)o.audio=o.correct?result.sentences.find(s=>s.id===g.sentenceId).audio:result.stage4AudioAlignment[g.id+'-'+o.id];
 ...
 if(path.resolve(String(file))===path.resolve('curriculum-workflow/audio-duration-report.json'))return write('curriculum-workflow/generated/L404-duration-report.json',data,...args);
...
process.argv=['node','script','--lesson','L404'];
