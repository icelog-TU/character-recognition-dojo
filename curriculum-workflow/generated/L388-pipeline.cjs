...
const action=process.argv[2],only=process.argv[3],dp='curriculum-workflow/drafts/L388-draft.json';
const shared=path.resolve('src/curriculum/sample-lessons.json');
const d=JSON.parse(read(dp,'utf8'));
const originalFetch=globalThis.fetch;
globalThis.fetch=async(...args)=>{
 if(String(args[0]).includes('/audio/speech')){const b=JSON.parse(args[1].body);b.instructions+=' 本課「念」讀ㄋㄧㄢˋ，第四聲與尾音完整。使用自然臺灣華語，只念輸入文字。';args[1]={...args[1],body:JSON.stringify(b)};}
 ...
 fs.mkdirSync('tmp/L388/transcripts',{recursive:true});write('tmp/L388/transcripts/'+name+'.json',JSON.stringify(raw,null,2)+'\n');
 const eq={'写':'寫','让':'讓','从':'從','个':'個','这':'這','画':'畫','后':'後','学':'學','们':'們','传':'傳','来':'來','连':'連','经':'經','图':'圖','给':'給','电':'電','帮':'幫','话':'話','饭':'飯','间':'間','里':'裡','声':'聲','这':'這','个':'個','妈':'媽','对':'對','说':'說','听':'聽'};
 Object.assign(eq,{'见':'見','书':'書','师':'師','脚':'腳'});
...
if(action==='generate'&&only){
 ...
 else if(only==='char-u5ff5'){d.sentences=[];d.sentenceGames=[];}
...
fs.writeFileSync=function(file,data,...args){
 ...
 if(path.resolve(String(file))===path.resolve('curriculum-workflow/audio-duration-report.json'))return write('curriculum-workflow/generated/L388-duration-report.json',data,...args);
...
process.argv=['node','script','--lesson','L388'];
