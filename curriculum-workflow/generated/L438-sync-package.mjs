...
const requestPath = 'curriculum-workflow/lesson-requests/L438.json';
const draft = JSON.parse(fs.readFileSync(draftPath, 'utf8'));
const request = JSON.parse(fs.readFileSync(requestPath, 'utf8'));
const status = process.argv[2] ?? draft.packageStatus;

draft.sentenceGames[4].options[0].audio = structuredClone(draft.sentences[4].audio);
draft.stage4AudioAlignment = {};
for (const game of draft.sentenceGames) {
  for (const part of ['prefix', 'suffix']) {
    if (game.teachAudio?.[`${part}Audio`]) {
      draft.stage4AudioAlignment[`${game.id}-${part}`] = game.teachAudio[`${part}Audio`];
    }
  }
  if (game.type === 'choose-pronunciation') {
    for (const option of game.options.filter((item) => !item.correct)) {
      draft.stage4AudioAlignment[`${game.id}-${option.id}`] = option.audio;
    }
  }
}
draft.packageStatus = status;
request.packageStatus = status;
if (fs.existsSync('curriculum-workflow/generated/L438-production-qa.md')) {
  draft.productionQA = fs.readFileSync('curriculum-workflow/generated/L438-production-qa.md', 'utf8');
}
...
fs.writeFileSync('curriculum-workflow/generated/L438-generation-packet.md', `# L438 習 — final production generation packet

packageStatus: ${status}

Source main: ${draft.sourceMainCommit}; formal L001-L435, last 助, 439 learned characters. Expanded locked allowedChars: 442. Production C owns this lesson-local package in parallel-c.
...
const coverageChars = ['習', '修', '補', '助', '互', '願'];
...
for (const item of [...draft.sentences, ...draft.sentenceGames.flatMap((game) => game.options ?? [])]) {
  for (const char of han(item.text)) if (!draft.allowedChars.includes(char)) errors.push(`Unlearned ${char}`);
}
...
const audio = [];
for (const src of [...tracks.map((track) => track.src), draft.charAudio.習]) {
...
const root = 'public/assets/lessons/L438';
let assetBytes = 0;
...
if (draft.allowedChars.length !== 442 || JSON.stringify(Object.values(coverage)) !== JSON.stringify([3, 2, 2, 2, 1, 1])) errors.push('coverage');
...
fs.writeFileSync('curriculum-workflow/generated/L438-package-audit.json', `${JSON.stringify(result, null, 2)}\n`);
