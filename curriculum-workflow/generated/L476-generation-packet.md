...
- 注音：故=ㄍㄨˋ
- 目標句數：5
- 教師備註：Teacher-approved L476 handoff. S02 final is the cold-weather deliberate opposite-speech scene. Fixed Xiaoguang, Xiaoyue and recurring sporty boy identities are mandatory. S04 has patterned front and blank back. No readable text or numbers. S04 空 is ㄎㄨㄥˋ; S03 重 ㄓㄨㄥˋ; S05 還 ㄏㄞˊ and 答 ㄉㄚˊ.
...
- 如果句子有數量，圖片必須清楚符合該數量。
- 優先使用明亮背景與清楚主體。
- 圖片目標路徑格式：`/assets/lessons/L476/images/L476-S01.webp`
...
- 本課每個新字都要建立一個單字音訊。
- 單字音訊目標路徑格式：`/assets/lessons/L476/audio/char-字.m4a`
- 聲音：自然、清楚、適合幼兒的臺灣華語。
- 讀 `spokenText`，不要讀顯示標點。
- 不要把句音訊做成逐字拼接。
- 句音訊目標路徑格式：`/assets/lessons/L476/audio/L476-S01.m4a`
...
```json
{
  "src": "/assets/lessons/L476/audio/L476-S01.m4a",
  "durationMs": 1200,
  "charTimings": [
...
```json
{
  ...
  "newChars": ["故"],
  "zhuyin": {"故":"ㄍㄨˋ"},
  "charAudio": {"故":"/assets/lessons/L476/audio/char-故.m4a"},
  "title": "故",
  "requiredRounds": 5,
  "sentences": [
    {
...
- Base main: 33ba62a69df2de88e8aaa49a2dd45c4347c540f7; formal production through L465 招; Release blockers R057/R058 and L470-L475
...
- Image QA: S01-S05 per-image style-lock/cast/semantics/no-readable-text PASS; no rejected image drafts
