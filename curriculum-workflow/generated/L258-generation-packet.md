...
- 注音：倒=ㄉㄠˇ
- 目標句數：5
- 教師備註：Production D lesson-local package. Teach 倒 as ㄉㄠˇ only in this lesson: 倒在, 倒頭就睡, 倒下, 打倒, 難倒. Do not introduce 倒水 ㄉㄠˋ in display text. Use 睡著, not 睡着. Release has resolved dependencies L251-L257 on latest origin/main.
...
- 如果句子有數量，圖片必須清楚符合該數量。
- 優先使用明亮背景與清楚主體。
- 圖片目標路徑格式：`/assets/lessons/L258/images/L258-S01.webp`
...
- 本課每個新字都要建立一個單字音訊。
- 單字音訊目標路徑格式：`/assets/lessons/L258/audio/char-u5012.m4a`
- 聲音：自然、清楚、適合幼兒的臺灣華語。
- 讀 `spokenText`，不要讀顯示標點。
- 不要把句音訊做成逐字拼接。
- 句音訊目標路徑格式：`/assets/lessons/L258/audio/L258-S01.m4a`
...
```json
{
  "src": "/assets/lessons/L258/audio/L258-S01.m4a",
  "durationMs": 1200,
  "charTimings": [
...
```json
{
  ...
  "newChars": ["倒"],
  "zhuyin": {"倒":"ㄉㄠˇ"},
  "charAudio": {"倒":"/assets/lessons/L258/audio/char-u5012.m4a"},
  "title": "倒",
  "requiredRounds": 5,
  "sentences": [
    {
