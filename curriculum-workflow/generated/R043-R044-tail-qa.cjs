...
for(const u of ['R043','R044']){
 ...
 for(const {id,a} of records){
  ...
  if(action==='metadata'){
   ...
   if(id==='R043-S01'){a.charTimings[4].startMs=1320;a.charTimings[4].endMs=1720;}
  }
 }
 if(action==='metadata'){
  for(const g of d.sentenceGames){if(g.teachAudio)for(const part of ['prefix','suffix']){const a=d.stage4AudioAlignment[g.id+'-'+part];g.teachAudio[part+'Audio']={src:a.src,durationMs:a.durationMs,charTimings:a.charTimings};}
...
fs.writeFileSync(`curriculum-workflow/generated/R043-R044-tail-${action}.json`,JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report,null,2));
