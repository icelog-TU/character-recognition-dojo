...
- 注音：被=ㄅㄟˋ
- 目標句數：5
- 教師備註：Production B L296 handoff. Complete package only; dependency-blocked until L290-L295 are in latest origin/main. Editor allowed-character sweep passed against origin/main L001-L287 plus provisionalLearnedChars ["耐","敢","當","然","怕","哭"] plus current new char 被. ImageNotes and prompts are production-only instructions and are not part of the learner-facing allowed-character gate.
...
- 本課每個新字都要建立一個單字音訊。
- 單字音訊目標路徑格式：`/assets/lessons/L296/audio/char-字.m4a`
- 聲音：自然、清楚、適合幼兒的臺灣華語。
...
```json
{
  ...
  "newChars": ["被"],
  "zhuyin": {"被":"ㄅㄟˋ"},
  "charAudio": {"被":"/assets/lessons/L296/audio/char-被.m4a"},
  "title": "被",
  "requiredRounds": 5,
  "sentences": [
    {
