...
- 注音：被=ㄅㄟˋ
- 目標句數：5
- 教師備註：Production B L296 handoff. Complete package only; release-integrated until L290-L295 are in latest origin/main. Editor allowed-character sweep passed against origin/main L001-L287 plus releaseIntegrationChars ["耐","敢","當","然","怕","哭"] plus current new char 被. ImageNotes and prompts are production-only instructions and are not part of the learner-facing allowed-character gate.
...
- 如果句子有數量，圖片必須清楚符合該數量。
- 優先使用明亮背景與清楚主體。
- 圖片目標路徑格式：`/assets/lessons/L296/images/L296-S01.webp`
...
- 本課每個新字都要建立一個單字音訊。
- 單字音訊目標路徑格式：`/assets/lessons/L296/audio/char-u88ab.m4a`
- 聲音：自然、清楚、適合幼兒的臺灣華語。
- 讀 `spokenText`，不要讀顯示標點。
- 不要把句音訊做成逐字拼接。
- 句音訊目標路徑格式：`/assets/lessons/L296/audio/L296-S01.m4a`
...
```json
{
  "src": "/assets/lessons/L296/audio/L296-S01.m4a",
  "durationMs": 1200,
  "charTimings": [
...
```json
{
  ...
  "newChars": ["被"],
  "zhuyin": {"被":"ㄅㄟˋ"},
  "charAudio": {"被":"/assets/lessons/L296/audio/char-u88ab.m4a"},
  "title": "被",
  "requiredRounds": 5,
  "sentences": [
    {
