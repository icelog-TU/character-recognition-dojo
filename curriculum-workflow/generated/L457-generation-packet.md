...
- 注音：實=ㄕˊ
- 目標句數：5
- 教師備註：Teacher-approved L457 handoff. Allowed ceiling is origin/main L001-L443 plus provisional 候演表現突 and new 實 (453 unique Han). Do not use 其實、實驗、實際. 實 is ㄕˊ alone, in 實現 and 果實; sentence S04 結實 is ㄐㄧㄝ ˙ㄕ via sentence override only. 突然 is ㄊㄨˊ ㄖㄢˊ. G02 suffix 現了 must remain exact and complete. G05 wrong choices require complete whole-sentence audio. Release remains ordered behind L444-L456 and the L450 R055/R056 milestone pair.
...
- 如果句子有數量，圖片必須清楚符合該數量。
- 優先使用明亮背景與清楚主體。
- 圖片目標路徑格式：`/assets/lessons/L457/images/L457-S01.webp`
...
- 本課每個新字都要建立一個單字音訊。
- 單字音訊目標路徑格式：`/assets/lessons/L457/audio/char-字.m4a`
- 聲音：自然、清楚、適合幼兒的臺灣華語。
- 讀 `spokenText`，不要讀顯示標點。
- 不要把句音訊做成逐字拼接。
- 句音訊目標路徑格式：`/assets/lessons/L457/audio/L457-S01.m4a`
...
```json
{
  "src": "/assets/lessons/L457/audio/L457-S01.m4a",
  "durationMs": 1200,
  "charTimings": [
...
```json
{
  ...
  "newChars": ["實"],
  "zhuyin": {"實":"ㄕˊ"},
  "charAudio": {"實":"/assets/lessons/L457/audio/char-實.m4a"},
  "title": "實",
  "requiredRounds": 5,
  "sentences": [
    {
...
- Immutable asset commit: d7d37c780aa739cc2b32bd207d10be0acc3df14f; pushed strict intake PASS
