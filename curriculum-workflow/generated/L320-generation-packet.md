...
- 注音：最=ㄗㄨㄟˋ
- 目標句數：5
- 教師備註：Production C complete-package handoff for L320 最. Approved sentences are fixed. Latest origin/main is complete through L317 粉; L318 哪 and L319 些 are not merged at claim time, so this package depends on L318 and L319 and uses 哪/些 as provisional learned characters. Learner-facing allowed-character sweep covers text, spokenText, displayLines, focusChar, and Stage 4 option text. Image notes and prompts are production instructions and are not part of the learned-character gate.
...
- 如果句子有數量，圖片必須清楚符合該數量。
- 優先使用明亮背景與清楚主體。
- 圖片目標路徑格式：`/assets/lessons/L320/images/L320-S01.webp`
...
- 本課每個新字都要建立一個單字音訊。
- 單字音訊目標路徑格式：`/assets/lessons/L320/audio/char-字.m4a`
- 聲音：自然、清楚、適合幼兒的臺灣華語。
- 讀 `spokenText`，不要讀顯示標點。
- 不要把句音訊做成逐字拼接。
- 句音訊目標路徑格式：`/assets/lessons/L320/audio/L320-S01.m4a`
...
```json
{
  "src": "/assets/lessons/L320/audio/L320-S01.m4a",
  "durationMs": 1200,
  "charTimings": [
...
```json
{
  ...
  "newChars": ["最"],
  "zhuyin": {"最":"ㄗㄨㄟˋ"},
  "charAudio": {"最":"/assets/lessons/L320/audio/char-最.m4a"},
  "title": "最",
  "requiredRounds": 5,
  "sentences": [
    {
...
Release integrated this package from `origin/codex/l320-complete-package` onto latest `origin/main` after L319. The package-local `dependsOnLessons` / `provisionalLearnedChars` references to L318 and L319 were cleared because L318 and L319 are already production lessons on the release base.

The final approved sentence and Stage 4 records below match `curriculum-workflow/drafts/L320-draft.json` and `curriculum-workflow/lesson-requests/L320.json`.

## Final Approved Sentences

| id | text | spokenText | focusChar | displayLines |
| --- | --- | --- | --- | --- |
| L320-S01 | 這些粉彩畫，哪一張最好看？ | 這些粉彩畫哪一張最好看 | 最 | 這些 / 粉彩畫， / 哪一張 / 最好看？ |
| L320-S02 | 有些衣服沒乾，要最後才能收。 | 有些衣服沒乾要最後才能收 | 最 | 有些衣服 / 沒乾， / 要最後 / 才能收。 |
| L320-S03 | 粉紅色最好再多一些。 | 粉紅色最好再多一些 | 最 | 粉紅色 / 最好再 / 多一些。 |
| L320-S04 | 哪一條路才能到學校？ | 哪一條路才能到學校 | 哪 | 哪一條路 / 才能到 / 學校？ |
| L320-S05 | 這一道是媽媽最拿手的菜。 | 這一道是媽媽最拿手的菜 | 最 | 這一道 / 是媽媽 / 最拿手的 / 菜。 |

## Final Stage 4 Games

| id | type | sentenceId | targetChar | notes |
| --- | --- | --- | --- | --- |
| L320-G01 | find-character | L320-S01 | 最 | Find 最 in sentence text. |
| L320-G02 | teach-character | L320-S03 | 最 | Prefix/suffix audio uses the sentence split around 最. |
| L320-G03 | missing-character | L320-S05 | 最 | Select missing 最. |
| L320-G04 | partial-order | L320-S04 | 哪 | Missing indexes 0-3; single-Han option cards 哪 / 一 / 條 / 路. |
| L320-G05 | choose-pronunciation | L320-S02 | 最 | Correct option is the approved sentence audio. |
