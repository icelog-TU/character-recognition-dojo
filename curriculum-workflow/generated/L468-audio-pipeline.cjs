...
globalThis.fetch = async (...args) => {
  if (String(args[0]).includes('/audio/speech')) {
    ...
    if (body.input === '星期日該輪到爸爸做飯了') { body.input = '星期日，該輪到爸爸做飯了。'; body.instructions = '只念輸入句子一次，使用自然流暢的臺灣華語。「星期日」讀 ㄒㄧㄥ ㄑㄧˊ ㄖˋ，其中期和奇同音，是自然清楚的第二聲，不可念成第一聲欺。「該」讀第一聲 ㄍㄞ。逗點只短停頓，句首句尾完整，不改字、不加字、不解釋。'; }
