...
globalThis.fetch = async (...args) => {
  if (String(args[0]).includes('/audio/speech')) {
    ...
    if (body.input === '星期日該輪到爸爸做飯了') { body.voice = 'cedar'; body.input = '星期日該輪到爸爸做飯了。'; body.instructions = '請用臺灣本地幼兒園老師自然說話的口音，只念「星期日該輪到爸爸做飯了」一次。整句使用臺灣華語的輕柔節奏與音色，不使用中國大陸普通話的捲舌、字正腔圓播音腔或強烈兒化。「星期日」自然連讀為 ㄒㄧㄥ ㄑㄧˊ ㄖˋ，期和奇同音，第二聲清楚上揚；不要念成「星七日」或第一聲「星欺日」。全句流暢、不刻意逐字頓讀；「該」讀第一聲。不得改字、加字或解釋。'; }
