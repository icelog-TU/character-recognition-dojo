...
- 注音：說=ㄕㄨㄛ
- 目標句數：5
- 教師備註：Production C complete package for L181. Latest origin/main production JSON is complete through L178 聲, so L181 depends on L179 聽 and L180 見. Use only latest production learned characters plus provisional 聽, 見 and current target 說. Approved sentences are fixed by handoff; do not introduce unlearned display text such as 話、請、跟、對、音、問候、禮貌.
...
- 如果句子有數量，圖片必須清楚符合該數量。
- 優先使用明亮背景與清楚主體。
- 圖片目標路徑格式：`/assets/lessons/L181/images/L181-S01.webp`
...
- 本課每個新字都要建立一個單字音訊。
- 單字音訊目標路徑格式：`/assets/lessons/L181/audio/char-字.m4a`
- 聲音：自然、清楚、適合幼兒的臺灣華語。
- 讀 `spokenText`，不要讀顯示標點。
- 不要把句音訊做成逐字拼接。
- 句音訊目標路徑格式：`/assets/lessons/L181/audio/L181-S01.m4a`
...
```json
{
  "src": "/assets/lessons/L181/audio/L181-S01.m4a",
  "durationMs": 1200,
  "charTimings": [
...
```json
{
  ...
  "newChars": ["說"],
  "zhuyin": {"說":"ㄕㄨㄛ"},
  "charAudio": {"說":"/assets/lessons/L181/audio/char-u8aaa.m4a"},
  "title": "說",
  "requiredRounds": 5,
  "sentences": [
    {
