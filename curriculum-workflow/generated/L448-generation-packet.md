...
- 注音：歌=ㄍㄜ
- 目標句數：5
- 教師備註：Formal approved L448 handoff. Base b594d6a9, production L438習/442 learned Han; allowed448 with provisional 操器樂音拍 and new歌. Release requires ordered L439-L447. 唱 reserved for L449; 首詞兒國 remain unlearned and absent. S03/S04 樂 is ㄩㄝˋ; S05 and G02 suffix 睡著 uses 著 ㄓㄠˊ. S01 after-performance applause, S02 request before photo, S05 girl already asleep. No image text or numbers. Production package only; Release owns shared JSON/planner/ledger integration and main merge.
...
- 如果句子有數量，圖片必須清楚符合該數量。
- 優先使用明亮背景與清楚主體。
- 圖片目標路徑格式：`/assets/lessons/L448/images/L448-S01.webp`
...
- 本課每個新字都要建立一個單字音訊。
- 單字音訊目標路徑格式：`/assets/lessons/L448/audio/char-字.m4a`
- 聲音：自然、清楚、適合幼兒的臺灣華語。
- 讀 `spokenText`，不要讀顯示標點。
- 不要把句音訊做成逐字拼接。
- 句音訊目標路徑格式：`/assets/lessons/L448/audio/L448-S01.m4a`
...
```json
{
  "src": "/assets/lessons/L448/audio/L448-S01.m4a",
  "durationMs": 1200,
  "charTimings": [
...
```json
{
  ...
  "newChars": ["歌"],
  "zhuyin": {"歌":"ㄍㄜ"},
  "charAudio": {"歌":"/assets/lessons/L448/audio/char-歌.m4a"},
  "title": "歌",
  "requiredRounds": 5,
  "sentences": [
    {
...
- 最終狀態：`dependency-blocked-asset-complete`
- 製作基準：`b594d6a952ff5c04d23160415ff81da279d7d025`；正式 production curriculum 到 L438「習」。
- 最新主線觀察：`a5779c4b` 只修復既有音訊，正式課程邊界仍為 L438。
- Release blockers：依序 L439-L447；Production 未寫入共用 production JSON、planner 或 ledger。
- 內容：五句核准句、五個 Stage 4 遊戲、五張 1024×1024 WebP、十個 AAC/M4A 音檔，以及完整字級 timing。
- 發音：S03/S04「樂」均為 ㄩㄝˋ；S05 與 G02 suffix「著」均為 ㄓㄠˊ；final processed assets 已用實際音訊輸入完成 acoustic AI 與 Whisper 對齊檢查。
- 圖片：五張 final WebP 已逐張檢視；style、固定角色、場景語意與無文字/數字均 PASS。
- 介面：real LessonPanel 於 390px isolated Vite harness 通過五關操作；十個音檔皆播放到 ended，領獎、下一課與回首頁 UI 正常，下一課回傳 `navigation next`。
...
- 教師圖片修復：重製 `L448-S05.webp`，將床修正為結構與透視一致的木床，並讓媽媽坐在床外清楚獨立的木椅；保留媽媽哼唱、女孩已睡著、夜間場景與既有角色畫風。Final 1024×1024 WebP visual QA PASS。
