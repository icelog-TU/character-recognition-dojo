...
- 注音：座=ㄗㄨㄛˋ
- 目標句數：5
- 教師備註：Teacher-approved final sentences from Production Handoff L278. Production note: G02 target is final Han character, so suffixText is empty; request uses prefix-only teachAudio to match existing schema.
...
- 如果句子有數量，圖片必須清楚符合該數量。
- 優先使用明亮背景與清楚主體。
- 圖片目標路徑格式：`/assets/lessons/L278/images/L278-S01.webp`
...
- 本課每個新字都要建立一個單字音訊。
- 單字音訊目標路徑格式：`/assets/lessons/L278/audio/char-字.m4a`
- 聲音：自然、清楚、適合幼兒的臺灣華語。
- 讀 `spokenText`，不要讀顯示標點。
- 不要把句音訊做成逐字拼接。
- 句音訊目標路徑格式：`/assets/lessons/L278/audio/L278-S01.m4a`
...
```json
{
  "src": "/assets/lessons/L278/audio/L278-S01.m4a",
  "durationMs": 1200,
  "charTimings": [
...
```json
{
  ...
  "newChars": ["座"],
  "zhuyin": {"座":"ㄗㄨㄛˋ"},
  "charAudio": {"座":"/assets/lessons/L278/audio/char-座.m4a"},
  "title": "座",
  "requiredRounds": 5,
  "sentences": [
    {
