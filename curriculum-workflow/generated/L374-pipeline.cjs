...
const draftPath = "curriculum-workflow/drafts/L374-draft.json";
...
globalThis.fetch = async (...args) => {
  ...
  fs.mkdirSync("tmp/L374/transcripts", {recursive:true});
  write("tmp/L374/transcripts/"+filename+".json", JSON.stringify(raw,null,2)+"\n");
  const equivalents = {"亲":"親","图":"圖","这":"這","条":"條","边":"邊","说":"說","话":"話","师":"師","给":"給","会":"會","来":"來"};
...
if (action === "align") {
  for (const game of draft.sentenceGames) {
    if (game.teachAudio?.prefixSrc) {
      draft.sentences.push({id:game.id+"-prefix",text:game.teachAudio.prefixText,spokenText:game.teachAudio.prefixText,approved:true,audio:{src:game.teachAudio.prefixSrc}});
    }
...
fs.writeFileSync = function(file, data, ...args) {
  ...
  if (path.resolve(String(file)) === path.resolve("curriculum-workflow/audio-duration-report.json")) {
    return write.call(this,"curriculum-workflow/generated/L374-duration-report.json",data,...args);
  }
...
process.argv = ["node","script","--lesson","L374"];
