...
- 注音：朵=ㄉㄨㄛˇ
- 目標句數：5
- 教師備註：教師核准 L069 新字：朵。L069 依賴 L068:花；目前花是老師明確指定的 provisional previous lesson character，但尚未進入 production main 時不得合併 L069 production。核准句子：水邊開著三朵花；要不要拿一朵花走？；這個包包上有很多花朵；我要拿走這個包包；大家左手都拿著一朵花。括號圖意：S02 是一個人手上很多花，要分給對方；S03 是在包包店看一個花朵圖案的包包；S04 續上文，付錢買包。只能使用 L001-L067 已學字、provisional L068 花、與本課新字朵。教師要求前三課每字至少 2 次，往前第四第五課每字至少 1 次；本組已達成：花 4、包 4、拿 3、著 2、左 1、朵 4。

## 已學字邊界

AI 必須把這份課程序列視為鎖定邊界。

- 前面已學字：一 二 三 人 個 大 的 小 手 我 有 山 上 下 你 水 在 高 很 家 和 隻 鳥 孩 指 看 女 飛 男 門 前 後 也 是 不 到 走 他 沒 裡 兩 狗 都 爸 媽 愛 書 可 會 這 吃 做 好 樣 要 更 邊 多 少 比 來 起 去 坐 站 開 左 著 拿 包
- 平行備課暫定已學字：花
- 本課新字：朵
- 顯示句子可用漢字：一 二 三 人 個 大 的 小 手 我 有 山 上 下 你 水 在 高 很 家 和 隻 鳥 孩 指 看 女 飛 男 門 前 後 也 是 不 到 走 他 沒 裡 兩 狗 都 爸 媽 愛 書 可 會 這 吃 做 好 樣 要 更 邊 多 少 比 來 起 去 坐 站 開 左 著 拿 包 花 朵
- 禁止：任何未列在上方的漢字。
- 只能使用臺灣華語與臺灣繁體字。
- 不要使用漢語拼音。
- 不要產生學習單式問答或測驗題。
...
- 如果句子有數量，圖片必須清楚符合該數量。
- 優先使用明亮背景與清楚主體。
- 圖片目標路徑格式：`/assets/lessons/L069/images/L069-S01.webp`
...
- 本課每個新字都要建立一個單字音訊。
- 單字音訊目標路徑格式：`/assets/lessons/L069/audio/char-字.m4a`
- 聲音：自然、清楚、適合幼兒的臺灣華語。
- 讀 `spokenText`，不要讀顯示標點。
- 不要把句音訊做成逐字拼接。
- 句音訊目標路徑格式：`/assets/lessons/L069/audio/L069-S01.m4a`
...
```json
{
  "src": "/assets/lessons/L069/audio/L069-S01.m4a",
  "durationMs": 1200,
  "charTimings": [
...
```json
{
  ...
  "newChars": ["朵"],
  "zhuyin": {"朵":"ㄉㄨㄛˇ"},
  "charAudio": {"朵":"/assets/lessons/L069/audio/char-朵.m4a"},
  "title": "朵",
  "requiredRounds": 5,
  "sentences": [
    {
