...
- 注音：又=ㄧㄡˋ
- 目標句數：5
- 教師備註：Teacher-approved final source for L167 又. Latest origin/main contains L164 擦; L167 depends on unmerged L165 先 and L166 洗, so do not merge production JSON until all dependencies are present in latest origin/main. Provisional learned chars are 先 and 洗. Use Taiwan Traditional Chinese and Taiwan zhuyin only; use 裡 not 裏. Do not add unlearned display characters such as 才, 洗完, 乾, 淨, 毛, 巾, 鞋櫃, 客, 廳, 亂, 髒, 歉, 道. Images must be square 1:1 compositions with centered meaning-bearing objects and safe margins, use L058 style anchors, and contain no text, numbers, signs, letters, zhuyin, labels, logos, or watermarks. Audio must be standalone OpenAI charAudio, whole-sentence AI audio, independent G02 prefix/suffix from exact fragments, and independent G05 wrong-choice whole-sentence audio.
...
- 如果句子有數量，圖片必須清楚符合該數量。
- 優先使用明亮背景與清楚主體。
- 圖片目標路徑格式：`/assets/lessons/L167/images/L167-S01.webp`
...
- 本課每個新字都要建立一個單字音訊。
- 單字音訊目標路徑格式：`/assets/lessons/L167/audio/char-字.m4a`
- 聲音：自然、清楚、適合幼兒的臺灣華語。
- 讀 `spokenText`，不要讀顯示標點。
- 不要把句音訊做成逐字拼接。
- 句音訊目標路徑格式：`/assets/lessons/L167/audio/L167-S01.m4a`
...
```json
{
  "src": "/assets/lessons/L167/audio/L167-S01.m4a",
  "durationMs": 1200,
  "charTimings": [
...
```json
{
  ...
  "newChars": ["又"],
  "zhuyin": {"又":"ㄧㄡˋ"},
  "charAudio": {"又":"/assets/lessons/L167/audio/char-又.m4a"},
  "title": "又",
  "requiredRounds": 5,
  "sentences": [
    {
