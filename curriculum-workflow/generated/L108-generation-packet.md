...
- 注音：點=ㄉㄧㄢˇ
- 目標句數：5
- 教師備註：Teacher-approved L108 sentence set for 點. Branch-only provisional work depends on L105:過, L106:分, and L107:幾 before production merge. Use Taiwan zhuyin and Taiwan wording. Do not use Hanyu pinyin. SpokenText must equal text with punctuation removed. Sentence images must use the L058 style anchors and must not contain readable text, numbers, labels, zhuyin, letters, signs, door plates, or UI. Production audio must use AI audio -> assets:audio -> assets:align:ai. Character audio char-u9ede.m4a must be standalone AI TTS for 點, not cut from sentence audio. G05 wrong-choice audio must be generated as complete wrong sentences, not spliced.
...
- 平行備課暫定已學字：過 分 幾
- 本課新字：點
- 顯示句子可用漢字：一 二 三 人 個 大 的 小 手 我 有 山 上 下 你 水 在 高 很 家 和 隻 鳥 孩 指 看 女 飛 男 門 前 後 也 是 不 到 走 他 沒 裡 兩 狗 都 爸 媽 愛 書 可 會 這 吃 做 好 樣 要 更 邊 多 少 比 來 起 去 坐 站 開 左 著 拿 包 花 朵 了 畫 出 學 路 誰 校 問 找 同 帶 筆 借 那 本 給 紙 心 放 把 桌 子 盒 掉 壞 眼 用 鏡 鼻 臉 紅 圓 太 難 得 過 分 幾 點
- 禁止：任何未列在上方的漢字。
- 只能使用臺灣華語與臺灣繁體字。
- 不要使用漢語拼音。
- 不要產生學習單式問答或測驗題。
...
- 如果句子有數量，圖片必須清楚符合該數量。
- 優先使用明亮背景與清楚主體。
- 圖片目標路徑格式：`/assets/lessons/L108/images/L108-S01.webp`
...
- 本課每個新字都要建立一個單字音訊。
- 單字音訊目標路徑格式：`/assets/lessons/L108/audio/char-字.m4a`
- 聲音：自然、清楚、適合幼兒的臺灣華語。
- 讀 `spokenText`，不要讀顯示標點。
- 不要把句音訊做成逐字拼接。
- 句音訊目標路徑格式：`/assets/lessons/L108/audio/L108-S01.m4a`
...
```json
{
  "src": "/assets/lessons/L108/audio/L108-S01.m4a",
  "durationMs": 1200,
  "charTimings": [
...
```json
{
  ...
  "newChars": ["點"],
  "zhuyin": {"點":"ㄉㄧㄢˇ"},
  "charAudio": {"點":"/assets/lessons/L108/audio/char-點.m4a"},
  "title": "點",
  "requiredRounds": 5,
  "sentences": [
    {
