...
const raw = execFileSync(ffmpeg, ['-v', 'error', '-i', 'public/assets/lessons/L463/audio/char-u671f.m4a', '-ar', '16000', '-ac', '1', '-f', 'f32le', 'pipe:1'], { maxBuffer: 8000000 });
...
const result = { name: 'char-u671f', points, summary: { firstHz: points[0]?.hz, minHz: Math.min(...points.map((point) => point.hz)), maxHz: Math.max(...points.map((point) => point.hz)), lastHz: points.at(-1)?.hz } };
fs.writeFileSync('curriculum-workflow/generated/L463-pitch-evidence.json', `${JSON.stringify(result, null, 2)}\n`);
