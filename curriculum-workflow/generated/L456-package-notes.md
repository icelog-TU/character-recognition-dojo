...
- Used the documented phonetic TTS rendering `圖` / `圖然` only to obtain Taiwan ㄊㄨˊ / ㄊㄨˊ ㄖㄢˊ. Learner-facing `突`, all five approved texts, `spokenText`, `displayLines`, focus characters, Stage 4 options, images, and other six audio files are unchanged. No syllable was cut, copied, or spliced.
...
All five WebPs retain Production's style-lock and cast PASS. S03, S04, G02 prefix/suffix, and both G05 wrong-choice M4As are byte-for-byte preserved from the Production checkpoint.
...
- Browser playback QA: all ten lesson audio files loaded and played to `ended` on 2026-09-18.
