...
const unit='L416';
const dp='curriculum-workflow/drafts/'+unit+'-draft.json',rp='curriculum-workflow/lesson-requests/'+unit+'.json',d=JSON.parse(fs.readFileSync(dp)),r=JSON.parse(fs.readFileSync(rp));
...
for(const file of fs.readdirSync('public/assets/lessons/'+unit+'/audio').filter(f=>f.endsWith('.m4a'))){const src='/assets/lessons/'+unit+'/audio/'+file,p='public'+src;
 ...
 const last=a.charTimings.at(-1);if(tail&&!file.startsWith('char-')&&durationMs-last.endMs>300&&speechEndMs>last.endMs){report.timingAdjustments.push({file,reason:'Final speech decay extends beyond ASR word endpoint; retain decoded final syllable envelope',before:{...last},afterEndMs:speechEndMs});last.endMs=speechEndMs;}
 ...
 if(!file.startsWith('char-')&&a.durationMs-last.endMs>300)report.issues.push(file+': tail '+(a.durationMs-last.endMs));
