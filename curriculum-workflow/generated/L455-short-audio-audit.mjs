...
const results = [];
for (const name of ['char-u73fe', 'L455-G02-prefix', 'L455-G02-suffix']) {
  const file = `public/assets/lessons/L455/audio/${name}.m4a`;
  const bytes = fs.readFileSync(file);
  const prompt = name === 'char-u73fe' ? '這是臺灣華語單一漢字音。課程候選字是「現」，請只轉寫實際聽到的漢字。' : null;
  const trials = [];
...
fs.writeFileSync('curriculum-workflow/generated/L455-short-audio-audit.json', `${JSON.stringify(results, null, 2)}\n`);
