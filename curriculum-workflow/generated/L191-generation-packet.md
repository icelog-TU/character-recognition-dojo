...
- 注音：發=ㄈㄚ
- 目標句數：5
- 教師備註：Production B complete package request for L191. Latest origin/main production JSON is complete through L190 火, so dependencies are satisfied and no provisional learned characters are needed. Use only latest production learned characters plus current target 發. Approved sentences are fixed by handoff.
...
- 如果句子有數量，圖片必須清楚符合該數量。
- 優先使用明亮背景與清楚主體。
- 圖片目標路徑格式：`/assets/lessons/L191/images/L191-S01.webp`
...
- 本課每個新字都要建立一個單字音訊。
- 單字音訊目標路徑格式：`/assets/lessons/L191/audio/char-u767c.m4a`
- 聲音：自然、清楚、適合幼兒的臺灣華語。
- 讀 `spokenText`，不要讀顯示標點。
- 不要把句音訊做成逐字拼接。
- 句音訊目標路徑格式：`/assets/lessons/L191/audio/L191-S01.m4a`
...
```json
{
  "src": "/assets/lessons/L191/audio/L191-S01.m4a",
  "durationMs": 1200,
  "charTimings": [
...
```json
{
  ...
  "newChars": ["發"],
  "zhuyin": {"發":"ㄈㄚ"},
  "charAudio": {"發":"/assets/lessons/L191/audio/char-u767c.m4a"},
  "title": "發",
  "requiredRounds": 5,
  "sentences": [
    {
