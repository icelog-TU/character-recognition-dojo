...
- 注音：發=ㄈㄚ
- 目標句數：5
- 教師備註：Production B complete package request for L191. Latest origin/main production JSON is complete through L190, so dependencies are satisfied and no provisional learned characters are needed. Approved sentences are fixed by handoff.
...
- 本課每個新字都要建立一個單字音訊。
- 單字音訊目標路徑格式：`/assets/lessons/L191/audio/char-字.m4a`
- 聲音：自然、清楚、適合幼兒的臺灣華語。
...
```json
{
  ...
  "newChars": ["發"],
  "zhuyin": {"發":"ㄈㄚ"},
  "charAudio": {"發":"/assets/lessons/L191/audio/char-發.m4a"},
  "title": "發",
  "requiredRounds": 5,
  "sentences": [
    {
