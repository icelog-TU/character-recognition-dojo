...
- 注音：滿=ㄇㄢˇ
- 目標句數：5
- 教師備註：Teacher-approved final source for L162. Latest origin/main is L158, so this package depends on unmerged L159 動, L160 葉, and L161 綠; do not merge production JSON until L159-L161 are present in latest origin/main. Use Taiwan Traditional Chinese and Taiwan zhuyin only; use 裡 not 裏. Do not use 滿 for 蠻好的. Do not add unlearned display characters such as 蠻, 圓, 客, 廳, 破, 瓶, 歉, 道, 搖, 落, 片, 秋, 黃, or 被. Images must use L058 style anchors, square 1:1 composition, centered subjects with safe margins, and contain no text, numbers, signs, letters, zhuyin, labels, logos, or watermarks. Audio must be standalone OpenAI charAudio, whole-sentence AI audio, independent G02 prefix/suffix, and independent G05 wrong-choice whole-sentence audio.
...
- 如果句子有數量，圖片必須清楚符合該數量。
- 優先使用明亮背景與清楚主體。
- 圖片目標路徑格式：`/assets/lessons/L162/images/L162-S01.webp`
...
- 本課每個新字都要建立一個單字音訊。
- 單字音訊目標路徑格式：`/assets/lessons/L162/audio/char-字.m4a`
- 聲音：自然、清楚、適合幼兒的臺灣華語。
- 讀 `spokenText`，不要讀顯示標點。
- 不要把句音訊做成逐字拼接。
- 句音訊目標路徑格式：`/assets/lessons/L162/audio/L162-S01.m4a`
...
```json
{
  "src": "/assets/lessons/L162/audio/L162-S01.m4a",
  "durationMs": 1200,
  "charTimings": [
...
```json
{
  ...
  "newChars": ["滿"],
  "zhuyin": {"滿":"ㄇㄢˇ"},
  "charAudio": {"滿":"/assets/lessons/L162/audio/char-滿.m4a"},
  "title": "滿",
  "requiredRounds": 5,
  "sentences": [
    {
