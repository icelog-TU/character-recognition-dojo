...
const requestPath = 'curriculum-workflow/lesson-requests/L450.json';
const draft = JSON.parse(fs.readFileSync(draftPath, 'utf8'));
const request = JSON.parse(fs.readFileSync(requestPath, 'utf8'));
const status = process.argv[2] ?? draft.packageStatus;

draft.sentenceGames[4].options[0].audio = structuredClone(draft.sentences.find((sentence) => sentence.id === 'L450-S02').audio);
...
if (fs.existsSync('curriculum-workflow/generated/L450-production-qa.md')) draft.productionQA = fs.readFileSync('curriculum-workflow/generated/L450-production-qa.md', 'utf8');
...
const audio = [];
for (const src of [...tracks.map((track) => track.src), draft.charAudio.舞]) {
...
const root = 'public/assets/lessons/L450';
let assetBytes = 0;
...
if (draft.allowedChars.length !== 448 || JSON.stringify(Object.values(coverage)) !== JSON.stringify([5, 2, 2, 2, 1, 1])) errors.push('coverage');
...
fs.writeFileSync('curriculum-workflow/generated/L450-package-audit.json', `${JSON.stringify(result, null, 2)}\n`);
