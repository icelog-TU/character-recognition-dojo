...
- 注音：早=ㄗㄠˇ
- 目標句數：5
- 教師備註：Teacher-approved final source for L208 早. Latest origin/main at production time includes L204 流 and L205 河, but L206 從 and L207 進 are not yet merged, so this is a dependency-blocked lesson-local package. Use Taiwan Traditional Chinese and Taiwan zhuyin only. Images must be square 1:1 compositions using approved L058 style anchors, with no text, numbers, signs, labels, logos, zhuyin, or watermarks. Audio must be standalone OpenAI charAudio, whole-sentence AI audio, independent G02 teach audio, and independent G05 wrong-choice whole-sentence audio.
...
- 如果句子有數量，圖片必須清楚符合該數量。
- 優先使用明亮背景與清楚主體。
- 圖片目標路徑格式：`/assets/lessons/L208/images/L208-S01.webp`
...
- 本課每個新字都要建立一個單字音訊。
- 單字音訊目標路徑格式：`/assets/lessons/L208/audio/char-字.m4a`
- 聲音：自然、清楚、適合幼兒的臺灣華語。
- 讀 `spokenText`，不要讀顯示標點。
- 不要把句音訊做成逐字拼接。
- 句音訊目標路徑格式：`/assets/lessons/L208/audio/L208-S01.m4a`
...
```json
{
  "src": "/assets/lessons/L208/audio/L208-S01.m4a",
  "durationMs": 1200,
  "charTimings": [
...
```json
{
  ...
  "newChars": ["早"],
  "zhuyin": {"早":"ㄗㄠˇ"},
  "charAudio": {"早":"/assets/lessons/L208/audio/char-早.m4a"},
  "title": "早",
  "requiredRounds": 5,
  "sentences": [
    {
