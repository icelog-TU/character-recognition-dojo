...
- 注音：活=ㄏㄨㄛˊ
- 目標句數：5
- 教師備註：Formal Production handoff locks all five sentence records and canonical Stage 4. 活 is ㄏㄨㄛˊ; 絕 is ㄐㄩㄝˊ; 期 is ㄑㄧˊ; 待 is ㄉㄞˋ. In S05 還 is ㄏㄞˊ and 活著 ends with neutral-tone ˙ㄓㄜ. G02 suffix begins exactly with 著 and must not omit it or repeat 活. The later reserved characters 該 and 應 and the unlearned 潑 and 復 are forbidden. Shared production JSON integration belongs to Release.
...
- 如果句子有數量，圖片必須清楚符合該數量。
- 優先使用明亮背景與清楚主體。
- 圖片目標路徑格式：`/assets/lessons/L467/images/L467-S01.webp`
...
- 本課每個新字都要建立一個單字音訊。
- 單字音訊目標路徑格式：`/assets/lessons/L467/audio/char-字.m4a`
- 聲音：自然、清楚、適合幼兒的臺灣華語。
- 讀 `spokenText`，不要讀顯示標點。
- 不要把句音訊做成逐字拼接。
- 句音訊目標路徑格式：`/assets/lessons/L467/audio/L467-S01.m4a`
...
```json
{
  "src": "/assets/lessons/L467/audio/L467-S01.m4a",
  "durationMs": 1200,
  "charTimings": [
...
```json
{
  ...
  "newChars": ["活"],
  "zhuyin": {"活":"ㄏㄨㄛˊ"},
  "charAudio": {"活":"/assets/lessons/L467/audio/char-活.m4a"},
  "title": "活",
  "requiredRounds": 5,
  "sentences": [
    {
