...
- 注音：澆=ㄐㄧㄠ
- 目標句數：5
- 教師備註：Production A complete-package handoff for L306 澆. Use approved sentences exactly. Latest origin/main must be verified before release. Learner-facing allowed-character sweep covers text, spokenText, displayLines, focusChar, and Stage 4 option text. Image notes and prompts are production instructions and are not part of the learned-character gate. Use natural Taiwan Mandarin for 澆水 and 澆花; do not use unnatural 澆樹 or unlearned display characters such as 熄, 壺, 照, 顧.
...
- 如果句子有數量，圖片必須清楚符合該數量。
- 優先使用明亮背景與清楚主體。
- 圖片目標路徑格式：`/assets/lessons/L306/images/L306-S01.webp`
...
- 本課每個新字都要建立一個單字音訊。
- 單字音訊目標路徑格式：`/assets/lessons/L306/audio/char-u6f86.m4a`
- 聲音：自然、清楚、適合幼兒的臺灣華語。
- 讀 `spokenText`，不要讀顯示標點。
- 不要把句音訊做成逐字拼接。
- 句音訊目標路徑格式：`/assets/lessons/L306/audio/L306-S01.m4a`
...
```json
{
  "src": "/assets/lessons/L306/audio/L306-S01.m4a",
  "durationMs": 1200,
  "charTimings": [
...
```json
{
  ...
  "newChars": ["澆"],
  "zhuyin": {"澆":"ㄐㄧㄠ"},
  "charAudio": {"澆":"/assets/lessons/L306/audio/char-u6f86.m4a"},
  "title": "澆",
  "requiredRounds": 5,
  "sentences": [
    {
