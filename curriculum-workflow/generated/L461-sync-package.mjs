...
const requestPath = 'curriculum-workflow/lesson-requests/L461.json';
const draft = JSON.parse(fs.readFileSync(draftPath, 'utf8'));
const request = JSON.parse(fs.readFileSync(requestPath, 'utf8'));
const status = process.argv[2] ?? draft.packageStatus;

draft.sentenceGames[4].options[1].audio = structuredClone(draft.sentences.find((sentence) => sentence.id === 'L461-S02').audio);
...
if (fs.existsSync('curriculum-workflow/generated/L461-production-qa.md')) draft.productionQA = fs.readFileSync('curriculum-workflow/generated/L461-production-qa.md', 'utf8');
...
fs.writeFileSync('curriculum-workflow/generated/L461-generation-packet.md', `# L461 文 — final production generation packet\n\npackageStatus: ${status}\n\nSource main: ${draft.sourceMainCommit}; formal L001-L446, last 音, 450 unique learned characters. Expanded locked allowedChars: 456. Production C owns this lesson-local package in parallel-c.\n\nRelease dependencies: ordinary lessons L456-L460 and review milestone R055/R056 after L450. The review pair is excluded from this package. Shared production JSON, planner export and ledger integration belong to Release.\n\n## Final approved request, sentence records, image prompts, and Stage 4\n\n\`\`\`json\n${JSON.stringify(request, null, 2)}\n\`\`\`\n\n## Production QA\n\n${draft.productionQA ?? 'Production in progress.'}\n`);

const han = (text) => [...text.matchAll(/\p{Script=Han}/gu)].map((match) => match[0]);
const errors = [];
const expected = ['中午，爸爸帶我去文具店。', '爸爸你看，我的作文也在其中。', '我其實沒吃午餐。', '風突然變大，把帽子吹走了。', '這本書只有圖畫，沒有文字。'];
const coverageChars = ['文', '午', '中', '其', '實', '突'];
...
let assetBytes = 0; for (const directory of ['images', 'audio']) for (const file of fs.readdirSync(path.join('public/assets/lessons/L461', directory))) assetBytes += fs.statSync(path.join('public/assets/lessons/L461', directory, file)).size; if (assetBytes > 2.5 * 1024 * 1024) errors.push('package size');
if (draft.allowedChars.length !== 456 || JSON.stringify(Object.values(coverage)) !== JSON.stringify([3, 2, 2, 2, 1, 1])) errors.push('coverage');
...
fs.writeFileSync('curriculum-workflow/generated/L461-package-audit.json', `${JSON.stringify(result, null, 2)}\n`); console.log(JSON.stringify(result, null, 2)); if (errors.length) process.exitCode = 1;
