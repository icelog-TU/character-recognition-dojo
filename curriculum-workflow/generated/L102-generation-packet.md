...
- 注音：太=ㄊㄞˋ
- 目標句數：5
- 教師備註：這是認字練功房 L102 課程，目標新字是「太」，注音為台灣注音 ㄊㄞˋ，不使用漢語拼音。老師已核准五句句子。只能使用已學字與 L102 新字「太」；因 L100「紅」與 L101「圓」尚未存在於 latest origin/main，本課只可把「紅」「圓」視為老師明確指定的 provisional learned chars 做平行準備。不要使用未學字：張、從、能、進、弄、睛、笑、陽。台灣用字，spokenText 必須等於 text 去掉標點。圖片使用 L058 風格參考，且不得出現文字、數字、招牌、標籤、注音、英文字母或浮水印。音檔使用 AI audio -> assets:audio -> assets:align:ai 標準流程；單字發音必須是獨立 AI 生成，不可從句子裁切。teachAudio prefix/suffix 也必須用精確片段獨立生成，不可從整句裁切。L102 依賴 L097「鏡」、L098「鼻」、L099「臉」、L100「紅」、L101「圓」，且 R003/R004 review modules 必須已依 SOP 插入 L090 後；目前 latest origin/main 已包含 L097-L099 與 R003/R004，但尚未包含 L100/L101，因此此課只做 branch-only 平行準備，不合併 production JSON。第 4 句製圖必須表現圖畫中的圓臉大鼻子，不是真人外貌的負面評價。第 5 句語意是孩子一個人出門或在外，媽媽不太放心，可畫孩子站在門邊或路邊，媽媽在旁邊擔心但溫和。

## 已學字邊界

AI 必須把這份課程序列視為鎖定邊界。

- 前面已學字：一 二 三 人 個 大 的 小 手 我 有 山 上 下 你 水 在 高 很 家 和 隻 鳥 孩 指 看 女 飛 男 門 前 後 也 是 不 到 走 他 沒 裡 兩 狗 都 爸 媽 愛 書 可 會 這 吃 做 好 樣 要 更 邊 多 少 比 來 起 去 坐 站 開 左 著 拿 包 花 朵 了 畫 出 學 路 誰 校 問 找 同 帶 筆 借 那 本 給 紙 心 放 把 桌 子 盒 掉 壞 眼 用 鏡 鼻 臉
- 平行備課暫定已學字：紅 圓
- 本課新字：太
- 顯示句子可用漢字：一 二 三 人 個 大 的 小 手 我 有 山 上 下 你 水 在 高 很 家 和 隻 鳥 孩 指 看 女 飛 男 門 前 後 也 是 不 到 走 他 沒 裡 兩 狗 都 爸 媽 愛 書 可 會 這 吃 做 好 樣 要 更 邊 多 少 比 來 起 去 坐 站 開 左 著 拿 包 花 朵 了 畫 出 學 路 誰 校 問 找 同 帶 筆 借 那 本 給 紙 心 放 把 桌 子 盒 掉 壞 眼 用 鏡 鼻 臉 紅 圓 太
- 禁止：任何未列在上方的漢字。
- 只能使用臺灣華語與臺灣繁體字。
- 不要使用漢語拼音。
- 不要產生學習單式問答或測驗題。
...
- 如果句子有數量，圖片必須清楚符合該數量。
- 優先使用明亮背景與清楚主體。
- 圖片目標路徑格式：`/assets/lessons/L102/images/L102-S01.webp`
...
- 本課每個新字都要建立一個單字音訊。
- 單字音訊目標路徑格式：`/assets/lessons/L102/audio/char-字.m4a`
- 聲音：自然、清楚、適合幼兒的臺灣華語。
- 讀 `spokenText`，不要讀顯示標點。
- 不要把句音訊做成逐字拼接。
- 句音訊目標路徑格式：`/assets/lessons/L102/audio/L102-S01.m4a`
...
```json
{
  "src": "/assets/lessons/L102/audio/L102-S01.m4a",
  "durationMs": 1200,
  "charTimings": [
...
```json
{
  ...
  "newChars": ["太"],
  "zhuyin": {"太":"ㄊㄞˋ"},
  "charAudio": {"太":"/assets/lessons/L102/audio/char-太.m4a"},
  "title": "太",
  "requiredRounds": 5,
  "sentences": [
    {
