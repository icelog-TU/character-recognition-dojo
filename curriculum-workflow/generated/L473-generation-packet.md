...
- 注音：反=ㄈㄢˇ
- 目標句數：5
- 教師備註：Teacher-approved L473 反. Exact sentences, display lines, image notes, Stage 4 order and options are locked by the Production handoff. S01 uses the final hamburger lunch scenario and fixed Xiaoyue reference. S04 speaker is mother and addressee is protagonist girl. No readable text or digit exceptions. Production owns the lesson-local package only and must not merge main.
...
- 如果句子有數量，圖片必須清楚符合該數量。
- 優先使用明亮背景與清楚主體。
- 圖片目標路徑格式：`/assets/lessons/L473/images/L473-S01.webp`
...
- 本課每個新字都要建立一個單字音訊。
- 單字音訊目標路徑格式：`/assets/lessons/L473/audio/char-字.m4a`
- 聲音：自然、清楚、適合幼兒的臺灣華語。
- 讀 `spokenText`，不要讀顯示標點。
- 不要把句音訊做成逐字拼接。
- 句音訊目標路徑格式：`/assets/lessons/L473/audio/L473-S01.m4a`
...
```json
{
  "src": "/assets/lessons/L473/audio/L473-S01.m4a",
  "durationMs": 1200,
  "charTimings": [
...
```json
{
  ...
  "newChars": ["反"],
  "zhuyin": {"反":"ㄈㄢˇ"},
  "charAudio": {"反":"/assets/lessons/L473/audio/char-反.m4a"},
  "title": "反",
  "requiredRounds": 5,
  "sentences": [
    {
