...
- 注音：敢=ㄍㄢˇ
- 目標句數：5
- 教師備註：Accepted Production handoff. Latest origin/main is complete through L287, so L286 皮 and L287 痛 are already learned; build lesson-local complete package only and block until L288-L290 are in main. Use origin/main L001-L287 plus provisional learned chars [受,忍,耐] and current target 敢 for allowed-character audit. Do not confuse 敢 with 趕. Injury/pain scenes must remain gentle, child-safe, non-bloody, and non-medical-horror.
...
- 如果句子有數量，圖片必須清楚符合該數量。
- 優先使用明亮背景與清楚主體。
- 圖片目標路徑格式：`/assets/lessons/L291/images/L291-S01.webp`
...
- 本課每個新字都要建立一個單字音訊。
- 單字音訊目標路徑格式：`/assets/lessons/L291/audio/char-u6562.m4a`
- 聲音：自然、清楚、適合幼兒的臺灣華語。
- 讀 `spokenText`，不要讀顯示標點。
- 不要把句音訊做成逐字拼接。
- 句音訊目標路徑格式：`/assets/lessons/L291/audio/L291-S01.m4a`
...
```json
{
  "src": "/assets/lessons/L291/audio/L291-S01.m4a",
  "durationMs": 1200,
  "charTimings": [
...
```json
{
  ...
  "newChars": ["敢"],
  "zhuyin": {"敢":"ㄍㄢˇ"},
  "charAudio": {"敢":"/assets/lessons/L291/audio/char-u6562.m4a"},
  "title": "敢",
  "requiredRounds": 5,
  "sentences": [
    {
