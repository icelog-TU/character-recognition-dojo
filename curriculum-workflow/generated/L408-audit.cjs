...
const d=read('curriculum-workflow/drafts/L408-draft.json'),r=read('curriculum-workflow/lesson-requests/L408.json');
const han=s=>[...s].filter(c=>/\p{Script=Han}/u.test(c));
const base=JSON.parse(cp.execFileSync('git',['show','0acb09a7984b22f2afd04d8add09ccff5468f842:src/curriculum/sample-lessons.json'],{maxBuffer:32e6}));
const allowed=[...new Set(base.lessons.flatMap(l=>l.newChars).concat(['越','加','減','法']))];
assert.equal(allowed.length,412);assert.deepEqual(r.allowedChars,allowed);
...
for(const w of ['法','減','加','越','弱','算'])coverage[w]=d.sentences.reduce((n,s)=>n+s.text.split(w).length-1,0);
assert.deepEqual(coverage,{'法':5,'減':2,'加':2,'越':2,'弱':1,'算':2});
...
assert.deepEqual(d.sentenceGames.map(g=>g.sentenceId),['L408-S01','L408-S02','L408-S03','L408-S05','L408-S04']);
assert.deepEqual(d.sentenceGames.map(g=>g.targetCharIndex),[1,4,5,4,0]);
assert.deepEqual(d.sentenceGames[3].missingIndexes,[7,8,9,10]);
assert.deepEqual(d.dependsOnLessons,['L405','L406','L407']);
assert.deepEqual(d.provisionalLearnedChars,['越','加','減']);
...
const g3=d.sentenceGames[2];assert.deepEqual(g3.options.map(o=>o.text),['法','方','去']);assert.equal(g3.options.filter(o=>o.correct).length,1);assert.deepEqual(g3.missingIndexes,[5]);
for(const c of d.newChars)audio.set(d.charAudio[c],{character:true});assert.equal(audio.size,10);
const out={unit:'L408',coverage,allowed:allowed.length,audio:[],images:[]};
...
if(process.argv.includes('--final')){
 ...
 const packet=fs.readFileSync('curriculum-workflow/generated/L408-generation-packet.md','utf8');
...
fs.writeFileSync('curriculum-workflow/generated/L408-technical-qa.json',JSON.stringify(out,null,2)+'\n');console.log(JSON.stringify(out,null,2));
