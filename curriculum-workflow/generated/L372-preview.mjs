...
curriculum.lessons.push(JSON.parse(fs.readFileSync('curriculum-workflow/drafts/L372-draft.json','utf8')));
// Local QA launch state only. Media, timing and gameplay implementation remain unchanged.
const preview={name:'l372-local-preview',enforce:'pre',load(id){if(id.replaceAll('\\','/').endsWith('/src/curriculum/sample-lessons.json'))return JSON.stringify(curriculum);},transform(code,id){if(!id.replaceAll('\\','/').endsWith('/src/App.tsx'))return;
return code.replace('const [freeBrowse, setFreeBrowse] = useState(false)','const [freeBrowse, setFreeBrowse] = useState(true)').replace('useState(initialProgress?.selectedOrder ?? nextOrder)','useState(372)').replace('const [lessonOpen, setLessonOpen] = useState(false)','const [lessonOpen, setLessonOpen] = useState(true)').replace('new Set(initialProgress?.completedOrders ?? [])','new Set([372])').replace('const initialDefaultStage = isReview ? 3 : 1','const initialDefaultStage = 3');}};
