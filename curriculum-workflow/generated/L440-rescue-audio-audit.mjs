...
const results = [];
for (const name of ["char-u904b", "L440-S05"]) {
  const file = `public/assets/lessons/L440/audio/${name}.m4a`;
...
fs.writeFileSync(
  "curriculum-workflow/generated/L440-rescue-audio-audit.json",
  `${JSON.stringify(results, null, 2)}\n`,
);
console.log(results.map((entry) => ({
  file: entry.file,
  texts: entry.trials.map((trial) => trial.text),
})));
