...
for(const [order,char,z] of [[381,'傳','ㄔㄨㄢˊ'],[382,'相','ㄒㄧㄤ'],[383,'信','ㄒㄧㄣˋ'],[384,'寫','ㄒㄧㄝˇ'],[385,'字','ㄗˋ']])curriculum.lessons.push({id:'L'+order,order,title:char,newChars:[char],zhuyin:{[char]:z},requiredRounds:5,sentences:[]});
curriculum.lessons.push(JSON.parse(fs.readFileSync('curriculum-workflow/drafts/L386-draft.json','utf8')));
const preview={name:'l386-local-preview',enforce:'pre',load(id){if(id.replaceAll('\\','/').endsWith('/src/curriculum/sample-lessons.json'))return JSON.stringify(curriculum);},transform(code,id){if(!id.replaceAll('\\','/').endsWith('/src/App.tsx'))return;return code.replace('const [freeBrowse, setFreeBrowse] = useState(false)','const [freeBrowse, setFreeBrowse] = useState(true)').replace('useState(initialProgress?.selectedOrder ?? nextOrder)','useState(386)').replace('const [lessonOpen, setLessonOpen] = useState(false)','const [lessonOpen, setLessonOpen] = useState(true)').replace('const initialDefaultStage = isReview ? 3 : 1','const initialDefaultStage = 1');}};
