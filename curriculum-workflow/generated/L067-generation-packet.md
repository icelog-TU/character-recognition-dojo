...
- 注音：包=ㄅㄠ
- 目標句數：5
- 教師備註：教師核准 L067 新字：包，注音 ㄅㄠ。教師核准五句：你拿著書包站起來；書包開著，看來包裡沒有書；他左手拿著包包站在門邊；你要去坐在爸爸的左邊；沒有人坐著，大家都站著。句長 4-12 個漢字，不含標點。本課新目標字包至少 3 次；前三課目標字拿、著、左至少各 2 次；往前第四課、第五課目標字開、站至少各 1 次。只能使用已學字加上本課新字。圖片要以 L058 三張作固定風格參考。音檔需 AI 句子音訊、teachAudio、Stage 4 option audio，以及 assets:align:ai 產生 charTimings。

## 已學字邊界

AI 必須把這份課程序列視為鎖定邊界。

- 前面已學字：一 二 三 人 個 大 的 小 手 我 有 山 上 下 你 水 在 高 很 家 和 隻 鳥 孩 指 看 女 飛 男 門 前 後 也 是 不 到 走 他 沒 裡 兩 狗 都 爸 媽 愛 書 可 會 這 吃 做 好 樣 要 更 邊 多 少 比 來 起 去 坐 站 開 左 著 拿
- 本課新字：包
- 顯示句子可用漢字：一 二 三 人 個 大 的 小 手 我 有 山 上 下 你 水 在 高 很 家 和 隻 鳥 孩 指 看 女 飛 男 門 前 後 也 是 不 到 走 他 沒 裡 兩 狗 都 爸 媽 愛 書 可 會 這 吃 做 好 樣 要 更 邊 多 少 比 來 起 去 坐 站 開 左 著 拿 包
- 禁止：任何未列在上方的漢字。
- 只能使用臺灣華語與臺灣繁體字。
- 不要使用漢語拼音。
- 不要產生學習單式問答或測驗題。
...
- 如果句子有數量，圖片必須清楚符合該數量。
- 優先使用明亮背景與清楚主體。
- 圖片目標路徑格式：`/assets/lessons/L067/images/L067-S01.webp`
...
- 本課每個新字都要建立一個單字音訊。
- 單字音訊目標路徑格式：`/assets/lessons/L067/audio/char-字.m4a`
- 聲音：自然、清楚、適合幼兒的臺灣華語。
- 讀 `spokenText`，不要讀顯示標點。
- 不要把句音訊做成逐字拼接。
- 句音訊目標路徑格式：`/assets/lessons/L067/audio/L067-S01.m4a`
...
```json
{
  "src": "/assets/lessons/L067/audio/L067-S01.m4a",
  "durationMs": 1200,
  "charTimings": [
...
```json
{
  ...
  "newChars": ["包"],
  "zhuyin": {"包":"ㄅㄠ"},
  "charAudio": {"包":"/assets/lessons/L067/audio/char-包.m4a"},
  "title": "包",
  "requiredRounds": 5,
  "sentences": [
    {
