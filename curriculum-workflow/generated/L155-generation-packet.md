...
- 注音：再=ㄗㄞˋ
- 目標句數：5
- 教師備註：Teacher-approved final source for L155. Latest origin/main is L153, so this package depends on unmerged L154 等; do not merge production JSON until L154 is present in latest origin/main. Use Taiwan Traditional Chinese and Taiwan zhuyin only; use 為 not 爲 and 裡 not 裏. Do not add unlearned display characters such as 喔, 快, 題, 目, 先, 自, 己, 公, or 具. Images must use L058 style anchors and contain no text, numbers, signs, route numbers, station names, license plates, zhuyin, labels, English, logos, or watermarks. Audio must be standalone OpenAI charAudio, whole-sentence AI audio, independent G02 prefix/suffix, and independent G05 wrong-choice whole-sentence audio.
...
- 如果句子有數量，圖片必須清楚符合該數量。
- 優先使用明亮背景與清楚主體。
- 圖片目標路徑格式：`/assets/lessons/L155/images/L155-S01.webp`
...
- 本課每個新字都要建立一個單字音訊。
- 單字音訊目標路徑格式：`/assets/lessons/L155/audio/char-字.m4a`
- 聲音：自然、清楚、適合幼兒的臺灣華語。
- 讀 `spokenText`，不要讀顯示標點。
- 不要把句音訊做成逐字拼接。
- 句音訊目標路徑格式：`/assets/lessons/L155/audio/L155-S01.m4a`
...
```json
{
  "src": "/assets/lessons/L155/audio/L155-S01.m4a",
  "durationMs": 1200,
  "charTimings": [
...
```json
{
  ...
  "newChars": ["再"],
  "zhuyin": {"再":"ㄗㄞˋ"},
  "charAudio": {"再":"/assets/lessons/L155/audio/char-再.m4a"},
  "title": "再等一下",
  "requiredRounds": 5,
  "sentences": [
    {
