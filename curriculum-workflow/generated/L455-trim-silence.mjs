...
const root = path.resolve('public/assets/lessons/L455/audio');
const report = [];
for (const name of fs.readdirSync(root).filter((item) => item.endsWith('.m4a') && !item.startsWith('char-'))) {
  ...
  const silenceStart = starts.at(-1); const silenceEnd = ends.at(-1); let keep = duration;
...
fs.writeFileSync('curriculum-workflow/generated/L455-silence-trim.json', `${JSON.stringify(report, null, 2)}\n`);
console.log(report);
