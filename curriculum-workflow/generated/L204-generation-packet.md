...
- 注音：流=ㄌㄧㄡˊ
- 目標句數：5
- 教師備註：Production D complete package request for L204. Latest origin/main production JSON is complete through L201 星. This lesson depends on L202 行 and L203 道 being merged before release. Use only latest production learned characters plus provisional learned characters 行、道 and current target 流. Approved final sentences, Stage 4 plan, square 1:1 L058-style image rules, and standard OpenAI audio rules are from the teacher handoff.
...
- 本課每個新字都要建立一個單字音訊。
- 單字音訊目標路徑格式：`/assets/lessons/L204/audio/char-字.m4a`
- 聲音：自然、清楚、適合幼兒的臺灣華語。
...
```json
{
  ...
  "newChars": ["流"],
  "zhuyin": {"流":"ㄌㄧㄡˊ"},
  "charAudio": {"流":"/assets/lessons/L204/audio/char-流.m4a"},
  "title": "流",
  "requiredRounds": 5,
  "sentences": [
    {
