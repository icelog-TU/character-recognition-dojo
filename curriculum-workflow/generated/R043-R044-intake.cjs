...
const h=t=>[...t].filter(c=>/\p{Script=Han}/u.test(c));const row=read('docs/PARALLEL_LESSON_REGISTRY.md').split('\n').find(l=>l.startsWith('| R043/R044 |'));
assert(row&&row.split('|')[3].trim()==='dependency-blocked-asset-complete');
for(const u of ['R043','R044']){
 ...
 assert(!block.test(JSON.stringify(d)+JSON.stringify(r)+packet+row));assert.equal(d.packageStatus,'asset-complete-package');assert.equal(r.packageStatus,d.packageStatus);assert.equal(d.afterLessonOrder,360);assert.deepEqual(d.targetLessonRange,{startOrder:331,endOrder:360});assert.equal(d.sentences.length,5);
 ...
 for(const s of d.sentences){exists(s.imageSrc);timing(s.audio,s.spokenText);const q=r.approvedSentences.find(q=>q.id===s.id);for(const k of ['text','spokenText','focusChar','displayLines','imageNotes']){assert.deepEqual(q[k],s[k]);assert(packet.includes(JSON.stringify(s[k])))};assert.equal(s.displayLines.join(''),s.text)}
