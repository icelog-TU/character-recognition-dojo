// Synchronize only the owned L386 package after media/QA changes.
const fs=require('fs'),cp=require('child_process'),assert=require('assert');
const dp='curriculum-workflow/drafts/L386-draft.json',rp='curriculum-workflow/lesson-requests/L386.json';
const d=JSON.parse(fs.readFileSync(dp,'utf8')),r=JSON.parse(fs.readFileSync(rp,'utf8'));
for(const g of d.sentenceGames){if(g.teachAudio){for(const part of ['prefix','suffix'])if(g.teachAudio[part+'Src'])g.teachAudio[part+'Audio']=d.stage4AudioAlignment[g.id+'-'+part];}if(g.type==='choose-pronunciation')for(const o of g.options)o.audio=o.correct?d.sentences.find(s=>s.id===g.sentenceId).audio:d.stage4AudioAlignment[g.id+'-'+o.id];}
...
const report=JSON.parse(fs.readFileSync('curriculum-workflow/generated/L386-duration-report.json','utf8'));
...
for(const [p,data] of [[dp,d],[rp,r],['curriculum-workflow/generated/L386-duration-report.json',report]])fs.writeFileSync(p,JSON.stringify(data,null,2)+'\n');
fs.writeFileSync('curriculum-workflow/generated/L386-generation-packet.md','# L386 generation packet\n\nPackage status: '+d.packageStatus+'\n\n'+r.teacherNotes+'\n\nQA evidence: L386-package-notes.md; mechanical and format reports accompany this packet.\n\n## Approved request\n\n```json\n'+JSON.stringify(r,null,2)+'\n```\n\n## Final lesson-local draft\n\n```json\n'+JSON.stringify(d,null,2)+'\n```\n');
