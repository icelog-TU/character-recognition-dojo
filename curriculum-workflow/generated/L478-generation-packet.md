...
- 注音：無=ㄨˊ
- 目標句數：5
- 教師備註：Formal Editor handoff assigned to Production A. Base origin/main 33ba62a6 with formal L001-L465 招 and 469 unique learned Han. L473-L477 provide provisional 反、而、且、故、緣; Release must also satisfy R057/R058 after L465. Allowed set has 475 unique Han. This is a normal single-character lesson: 無緣無故 introduces only 無. S01 must show the ball under the cabinet as the reason for barking; S02 is the rainy stopped hike while S04 is a separate sunny hike; S03 automatic drone flight remains supervised. No readable image text or numbers. Production package only; Release owns shared production integration and main merge.
...
- 如果句子有數量，圖片必須清楚符合該數量。
- 優先使用明亮背景與清楚主體。
- 圖片目標路徑格式：`/assets/lessons/L478/images/L478-S01.webp`
...
- 本課每個新字都要建立一個單字音訊。
- 單字音訊目標路徑格式：`/assets/lessons/L478/audio/char-字.m4a`
- 聲音：自然、清楚、適合幼兒的臺灣華語。
- 讀 `spokenText`，不要讀顯示標點。
- 不要把句音訊做成逐字拼接。
- 句音訊目標路徑格式：`/assets/lessons/L478/audio/L478-S01.m4a`
...
```json
{
  "src": "/assets/lessons/L478/audio/L478-S01.m4a",
  "durationMs": 1200,
  "charTimings": [
...
```json
{
  ...
  "newChars": ["無"],
  "zhuyin": {"無":"ㄨˊ"},
  "charAudio": {"無":"/assets/lessons/L478/audio/char-無.m4a"},
  "title": "無",
  "requiredRounds": 5,
  "sentences": [
    {
