...
const raw = execFileSync(ffmpeg, ['-v', 'error', '-i', 'public/assets/lessons/L455/audio/char-u73fe.m4a', '-ar', '16000', '-ac', '1', '-f', 'f32le', 'pipe:1'], { maxBuffer: 8000000 });
...
const result = { name: 'char-u73fe', points, summary: { firstHz: points[0]?.hz, minHz: Math.min(...points.map((point) => point.hz)), maxHz: Math.max(...points.map((point) => point.hz)), lastHz: points.at(-1)?.hz } };
fs.writeFileSync('curriculum-workflow/generated/L455-pitch-evidence.json', `${JSON.stringify(result, null, 2)}\n`);
