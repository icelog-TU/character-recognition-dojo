...
const d=read('curriculum-workflow/drafts/L399-draft.json'),r=read('curriculum-workflow/lesson-requests/L399.json');
const han=s=>[...s].filter(c=>/\p{Script=Han}/u.test(c));
const base=JSON.parse(cp.execFileSync('git',['show','23cae19103e760f23700f5ff90d72fbd2e59bfc6:src/curriculum/sample-lessons.json'],{maxBuffer:32e6}));
const allowed=[...new Set(base.lessons.flatMap(l=>l.newChars).concat(['翻','印','單','雙','選','或']))];
assert.equal(allowed.length,400);assert.deepEqual(r.allowedChars,allowed);
...
for(const w of ['或','選','雙','單','印','翻'])coverage[w]=d.sentences.reduce((n,s)=>n+s.text.split(w).length-1,0);
assert.deepEqual(coverage,{'或':4,'選':2,'雙':2,'單':2,'印':1,'翻':1});
...
assert.deepEqual(d.sentenceGames.map(g=>g.sentenceId),['L399-S01','L399-S02','L399-S04','L399-S03','L399-S05']);
assert.deepEqual(d.sentenceGames.map(g=>g.targetCharIndex),[5,6,5,2,7]);
assert.deepEqual(d.sentenceGames[3].missingIndexes,[5,6,7,8]);
assert.deepEqual(d.dependsOnLessons,['L394','L395','L396','L397','L398']);
assert.deepEqual(d.provisionalLearnedChars,['翻','印','單','雙','選']);
...
for(const g of d.sentenceGames){
 ...
 for(const o of g.options??[]){
  ...
  if(g.type==='choose-pronunciation'){
   assert.equal(han(o.text).length,11);assert.equal(han(o.text).join(''),o.spokenText);
...
const g3=d.sentenceGames[2];assert.deepEqual(g3.options.map(o=>o.text),['或','和','比']);assert.equal(g3.options.filter(o=>o.correct).length,1);assert.deepEqual(g3.missingIndexes,[5]);
for(const c of d.newChars)audio.set(d.charAudio[c],{character:true});assert.equal(audio.size,10);
const out={unit:'L399',coverage,allowed:allowed.length,audio:[],images:[]};
...
if(process.argv.includes('--final')){
 ...
 const packet=fs.readFileSync('curriculum-workflow/generated/L399-generation-packet.md','utf8');
...
fs.writeFileSync('curriculum-workflow/generated/L399-technical-qa.json',JSON.stringify(out,null,2)+'\n');console.log(JSON.stringify(out,null,2));
