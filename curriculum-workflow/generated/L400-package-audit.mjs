...
const request = read("curriculum-workflow/lesson-requests/L400.json");
const draft = read("curriculum-workflow/drafts/L400-draft.json");
const packet = fs.readFileSync("curriculum-workflow/generated/L400-generation-packet.md","utf8");
...
const targets={者:3,或:2,選:2,雙:2,單:1,印:1};
...
assert(fs.existsSync("public"+draft.charAudio["者"]));
console.log(JSON.stringify({unit:"L400",result:"PASS",allowed:allowed.size,coverage,checks:["approved records","display lines","paths","timing spans and tails","fixed game order","single-Han options","three-choice G03","G02 exact fragments","G05 full texts"]},null,2));
