...
for(const [order,char,z] of [[405,'越','ㄩㄝˋ'],[406,'加','ㄐㄧㄚ'],[407,'減','ㄐㄧㄢˇ'],[408,'法','ㄈㄚˇ'],[409,'辦','ㄅㄢˋ']])if(!curriculum.lessons.some(l=>l.order===order))curriculum.lessons.push({id:'L'+order,order,title:char,newChars:[char],zhuyin:{[char]:z},requiredRounds:5,sentences:[]});
curriculum.lessons.push(JSON.parse(fs.readFileSync('curriculum-workflow/drafts/L410-draft.json','utf8')));
const preview={name:'l410-local-preview',enforce:'pre',load(id){if(id.replaceAll('\\','/').endsWith('/src/curriculum/sample-lessons.json'))return JSON.stringify(curriculum);},transform(code,id){if(!id.replaceAll('\\','/').endsWith('/src/App.tsx'))return;return code.replace('const [freeBrowse, setFreeBrowse] = useState(false)','const [freeBrowse, setFreeBrowse] = useState(true)').replace('useState(initialProgress?.selectedOrder ?? nextOrder)','useState(410)').replace('const [lessonOpen, setLessonOpen] = useState(false)','const [lessonOpen, setLessonOpen] = useState(true)').replace('const initialDefaultStage = isReview ? 3 : 1','const initialDefaultStage = 1');}};
