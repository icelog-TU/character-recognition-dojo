...
const request = read("curriculum-workflow/lesson-requests/L409.json");
const draft = read("curriculum-workflow/drafts/L409-draft.json");
const packet = fs.readFileSync("curriculum-workflow/generated/L409-generation-packet.md","utf8");
...
const targets={辦:3,法:2,減:2,加:2,越:1,弱:1};
...
assert(fs.existsSync("public"+draft.charAudio["辦"]));
console.log(JSON.stringify({unit:"L409",result:"PASS",allowed:allowed.size,coverage,checks:["approved records","display lines","paths","timing spans and tails","fixed game order","single-Han options","three-choice G03","G02 exact fragments","G05 full texts"]},null,2));
