...
- 注音：出=ㄔㄨ
- 目標句數：5
- 教師備註：Teacher approved L073 new character `出` and the five final sentences in chat. Current latest origin/main is L069 `朵`; L070 `了`, L071/L072 including `畫` are not all production truth yet, so prepare L073 assets only and do not merge to production until L070-L072 are merged and rechecked. `出` should be read ㄔㄨ. Keep `畫` as the traditional character and use Taiwan zhuyin only.
...
- 如果句子有數量，圖片必須清楚符合該數量。
- 優先使用明亮背景與清楚主體。
- 圖片目標路徑格式：`/assets/lessons/L073/images/L073-S01.webp`
...
- 本課每個新字都要建立一個單字音訊。
- 單字音訊目標路徑格式：`/assets/lessons/L073/audio/char-字.m4a`
- 聲音：自然、清楚、適合幼兒的臺灣華語。
- 讀 `spokenText`，不要讀顯示標點。
- 不要把句音訊做成逐字拼接。
- 句音訊目標路徑格式：`/assets/lessons/L073/audio/L073-S01.m4a`
...
```json
{
  "src": "/assets/lessons/L073/audio/L073-S01.m4a",
  "durationMs": 1200,
  "charTimings": [
...
```json
{
  ...
  "newChars": ["出"],
  "zhuyin": {"出":"ㄔㄨ"},
  "charAudio": {"出":"/assets/lessons/L073/audio/char-出.m4a"},
  "title": "出",
  "requiredRounds": 5,
  "sentences": [
    {
