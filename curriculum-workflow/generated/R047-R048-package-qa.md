...
Final-draft preview recheck: shutting down the old long-running preview exposed a runTeachCharacterIntro charTimings.find exception. The old fixture held pre-alignment data in memory. Restarted from final drafts and used the fixture-only game-index selector to navigate directly to G02 (this selector does not simulate completion). R047 and R048 both reached the real ready prompt 請按住紅框的字不放 after intro playback; no further timing exception appeared in the new server log. Sustained microphone recording/stitched replay remains unverified with the available instantaneous-click API. Final package audio bytes and technical results are unchanged. HEAD and pushed-origin intake both passed for both reviews at fd67e4d3; this follow-up only improves the QA fixture and records the recheck, with intake repeated for its final tip.
